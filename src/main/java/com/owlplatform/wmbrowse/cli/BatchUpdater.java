 ...
 */
package com.owlplatform.wmbrowse.cli;
 ...
 * @author Robert Moore
 */
public class BatchUpdater extends Thread {

  /**
   * Logger for this class.
   */
  private static final Logger log = LoggerFactory.getLogger(BatchUpdater.class);

  /**
   * Title for this app.
   */
  public static final String TITLE = "Owl Platform WM Batch Updater";
  ...
  public static void main(String[] args) {
    ...
    BatchUpdater b = new BatchUpdater(wmHost, origin, solverPort, clientPort);
    b.start();
  }
   ...
   */
  public BatchUpdater(final String wmHost, final String origin,
  ...
  public void run() {
    ...
    try {
      ...
      // First loop for data types
      while (this.keepRunning) {
        System.out.print("Please enter a data type, or an empty line to end: ");
        String in = this.userIn.readLine().trim();
        if (in.length() == 0) {
          break;
        }
        types.add(in);
      ...
      while (this.keepRunning) {
        ...
        String id = this.userIn.readLine().trim();

        for (String attr : types) {
          this.handleCommand("update " + id + " " + attr);
        }
      ...
      log.error("Exception reading user input.", ioe);
    }
  ...
  protected void delete(final String command) {
    ...
    if (components.size() == 1) {
      ...
      System.out.println("Deleted attribute \"" + components.get(1)
          + "\" for \"" + components.get(0) + "\".");
    }
  }
  ...
  protected void recursiveCopy(final String source, final String destination) {
    ...
    try {
      while (!resp.isComplete() && !resp.isError()) {
        ...
        int numCopies = this.copyAttributes(origState.getState(source),
            destination);
        if (numCopies < 0) {
          System.out
              .println("Error while copying one or more Attributes. Aborting.");
          break;
        }
  ...
  protected void shallowCopy(final String source, final String destination) {
    ...
    if (numAttr >= 0) {
      ...
      System.out
          .println("An error has occurred. One or more Attributes was not copied.");
    }
  }
